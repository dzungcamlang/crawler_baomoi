...
class AsnycGrab(object):
    ...
    def __parse_results(self, url, html, index):
        ...
        try:
            ...
            if not check_404(soup):
                title, source, tags, category, time, sum, content = find_objects(
                    soup)
        ...
        if title:
            # print(index)
            self.write_text(title, source, tags, category,
                            time, sum, content, url, index)
            self.count_file()
    ...
    async def handle_tasks(self, task_id, work_queue):
        while not work_queue.empty():
            ...
            try:
                ...
                logging.exception('Error for {}'.format(
                    current_index), exc_info=True)
    ...
    def eventloop(self):
        ...
        tasks = [self.handle_tasks(task_id, q, )
...
if __name__ == '__main__':
    ...
    startIndex = 11111111
    indexList = range(startIndex, startIndex + 50)
    async_example = AsnycGrab(indexList, 5)
    async_example.eventloop()
